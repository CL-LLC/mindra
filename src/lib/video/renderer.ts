...
/**
 * Generate BASE Editly configuration (Balanced architecture)
 * This creates a reusable video WITHOUT burned-in affirmation text
 * Affirmations are delivered via overlay at playback time
 */
export function generateBaseEditlyConfig(
  ...
  const editlyConfig = {
    ...
    clips: scenes.map((scene) => ({
      ...
      layers: [
        // Image/video layer only - NO text overlay
        ...(scene.imageUrl ? [{
          type: 'image' as const,
          path: scene.imageUrl,
          ...(scene.transition === 'zoom' && { zoom: [1, 1.2] }),
          ...(scene.transition === 'pan' && { position: ['left', 'right'] as const }),
        }] : [{
          type: 'fill-color' as const,
          color: '#0f172a',
        }]),
        
        // Ken Burns effect (subtle zoom) - visual interest without text
        {
 ...
 * This is stored alongside the video and used by the player
 * 
