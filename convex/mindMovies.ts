...
export const list = query({
  ...
  handler: async (ctx) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) return [];
    const [ready, rendering, draft] = await Promise.all([
      ctx.db.query("mindMovies").withIndex("by_userId_status", (q) => q.eq("userId", userId).eq("status", "ready")).order("desc").take(20),
      ctx.db.query("mindMovies").withIndex("by_userId_status", (q) => q.eq("userId", userId).eq("status", "rendering")).order("desc").take(20),
      ctx.db.query("mindMovies").withIndex("by_userId_status", (q) => q.eq("userId", userId).eq("status", "draft")).order("desc").take(20),
    ]);
    return [...ready, ...rendering, ...draft]
...
export const getActive = query({ args: {}, handler: async (ctx) => { const userId = await getAuthUserId(ctx); if (!userId) return []; const [ready, rendering, draft] = await Promise.all([ctx.db.query("mindMovies").withIndex("by_userId_status", (q) => q.eq("userId", userId).eq("status", "ready")).take(10), ctx.db.query("mindMovies").withIndex("by_userId_status", (q) => q.eq("userId", userId).eq("status", "rendering")).take(10), ctx.db.query("mindMovies").withIndex("by_userId_status", (q) => q.eq("userId", userId).eq("status", "draft")).take(10)]); return [...ready, ...rendering, ...draft].sort((a, b) => b.createdAt - a.createdAt).slice(0, 10); } });
